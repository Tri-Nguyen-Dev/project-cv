...
function Products() {
  ...
  const { _totalRows, _page, _limit } = products.pagination
    ? products.pagination
    : {};
  ...
  return (
    <div className="products">
      <div className="grid wide">
        <BreadcrumbCom page="Product" />
      </div>

      {loading ? (
        <div className="tab-loading" style={{ textAlign: "center" }}>
          <Spin size="large" />
        </div>
      ) : (
        ""
      ...
      {!loading && (
        <div className="grid wide">
          <div className="product__heading">
            ...
            <div className="product__filters">
              <span className="product__number--page">
                SHOWING {_page === 1 ? 1 : _page * _limit - _limit} -{" "}
                {_page < totalPage ? _limit * _page : _totalRows} OF{" "}
                {_totalRows} ITEM(S)
              </span>
              <ul className="product__grid">
                {listGrid.map((item, index) => {
                  return (
                    <li
                      onClick={() => handelChangeGrid(index)}
                      className={index === indexGrid ? "active" : ""}
                      key={index}
                    >
              ...
              <div className="product__sort" onClick={handleShowSort}>
                ...
                <ul
                  className={
                    !isShowSort
                      ? "peoduct__sort--list"
                      : "peoduct__sort--list active"
                  }
                >
                  {listSort.map((item, index) => {
                    return (
                      <li key={index} onClick={handleClickSort}>
                        {item}
                      </li>
                    );
                  })}
              ...
              {products.data
                ? products.data.map((item, index) => {
                    return (
                      <div
                        key={index}
                        className={indexGrid === 0 ? "col l-3" : "col l-4"}
                      >
          ...
          <div>
            <Paginate
              paginate={products.pagination ? products.pagination : {}}
